...
export const SidebarLink: FC<{ to: string, activeOnlyWhenExact: boolean, children: React.ReactNode }> = ({ to, activeOnlyWhenExact, children }) => (
  <Route
    path={to}
    ...
    children={({ match, location }) => {
      const child = React.Children.map(children, mappedChild => {
        ...
        if (!React.isValidElement<EnrichedChildren>(children))
          return children
